<?php
   ...
   @$nom=htmlspecialchars(str_replace("'","¬",$donnees["nom"]));
   @$createur=$donnees["createur"];
   @$id=$donnees["evenement"];
   @$affiche=$_FILES["file"];
   ...
   if(isset($valider)){
      ...
      elseif(!in_array($type_affiche,$extensions)) $erreur="affiche non soumise ou au mauvais format(Seuls png,jpg et jpeg sont acceptés)";
      else{
...
<html>
   ...
   <body>
      ...
      <form name="fo" method="post" action="" enctype="multipart/form-data">
         <input type="text" name="nom" placeholder="Nom" value="<?php echo $donnees['nom']?>" /><br />
         <input type="textarea" name="description" placeholder="Faites une brève description de votre evenement." value="<?php echo $donnees['description_evenement']?>" /><br />
         ...
         <input type="file" name="file" id="file" accept="image/*" hidden onchange="previewImage();" value=""  >
         <br />
         <img id="preview" src="affiches/<?php echo $donnees['id_affiche'] ?>"><br />
         <label for="date_debut">Date debut</label>

<input type="date" name="date_debut" id="date_debut" min="today()" value="<?php echo $donnees['date_debut']?>"
       >
         <br />
...
<input type="date" name="date_fin" id="date_fin" value="<?php echo $donnees['date_fin']?>"
    ...
    <label for="heure_debut">Heure de debut</label>
         <input type="time" name="heure_debut" id="heure_debut" value="<?php echo $donnees['heure_debut']?>"/><br />
         <label for="heure_debut">Heure de fin</label>
         <input type="time" name="heure_fin" id="heure_fin" value="<?php echo $donnees['heure_fin']?>" /><br />
         <label for="localisation">Localisation(lien)</label>
         <input type="text" name="localisation" placeholder="localisation(sous forme de lien)" value="<?php echo $donnees['localisation']?>" id="localisation"/><br />
         <label for="prix">Prix d'entrée en FCFA</label>
         <input type="int" name="prix" placeholder="ex: 2000" id="prix" value="<?php echo $donnees['prix']?>" /><br />
         <label for="nbr_stand">Nombre de stands</label>
         <input type="int" name="nbr_stand" placeholder="ex: 20" id="nbr_stand" value="<?php echo $donnees['max_stand']?>" /><br />
         <label for="nbr_stand">Prix du stand</label>
         <input type="int" name="prix_stand" placeholder="ex: 2000" id="prix_stand" value="<?php echo $donnees['prix_stand']?>" /><br />
